...
next_section:
    ...
    cmp di, 1024                 ; read 1024 sectors (512 KB)?
    je done                      ; done reading

    ; handle sector/head/cylinder rollover (CHS logic)
    mov al, [sector_num]
    cmp al, 19                   ; if sector_num > 18, we must reset and increment head
    jbe skip_head_update

    mov byte [sector_num], 1     ; reset sector to 1
    inc dh                       ; increment head

    cmp dh, 2                    ; if head > 1, move to next cylinder
    jb skip_head_update
